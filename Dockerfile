...
RUN apk add --no-cache \
    ...
    libnl3-dev libnl3-static cmake ninja python3

WORKDIR /src

# 2. Build RDMA-CORE statically (Required for portable RDMA support)
# -DNO_MAN_PAGES=1 avoids documentation build errors
RUN curl -L https://github.com/linux-rdma/rdma-core/releases/download/v49.1/rdma-core-49.1.tar.gz | tar -xz && \
    cd rdma-core-49.1 && \
    cmake -GNinja \
...
# 3. Build KEYUTILS from source (Ensure libkeyutils.a exists)
...
# 4. Build CUSTOM LIBTIRPC (Forced IPv6, No GSS)
...
# 5. Download and Build NFS-UTILS 2.6.4
...
# 6. Configure nfs-utils with IPv6 and RDMA enabled
RUN ./configure \
...
# 7. Build nfs-utils
# LIBS includes static RDMA and Netlink libs for portability
RUN make -j$(nproc) \
    LDFLAGS="-all-static" \
    CFLAGS="-static -Wno-error" \
    LIBS="-ltirpc -lrdmacm -libverbs -lnl-route-3 -lnl-3 -levent_core -levent -lmount -lblkid -luuid /usr/lib/libdevmapper.a /usr/lib/libudev.a /usr/lib/libaio.a -lsqlite3 -lattr -lz -lpthread /usr/lib/libkeyutils.a"

RUN make install DESTDIR=/install

# 8. Build RPCBIND statically (Required for mountd to register)
WORKDIR /src
RUN curl -L https://downloads.sourceforge.net/project/rpcbind/rpcbind/1.2.6/rpcbind-1.2.6.tar.bz2 | tar -xj && \
    cd rpcbind-1.2.6 && \
    ./configure --prefix=/usr --sbindir=/sbin --bindir=/sbin --with-rpcuser=root --enable-warmstarts --enable-ipv6 --with-tirpcinclude=/usr/include/tirpc --without-systemdsystemunitdir && \
    make LDFLAGS="-static" LIBS="-ltirpc -lpthread" && \
    make install DESTDIR=/install

# 9. Consolidation (Flattens files directly into /final for your --output command)
RUN mkdir -p /final && \
    # Find and strip all ELF binaries, then copy them directly into /final
    find /install -type f -executable -exec sh -c 'file "$1" | grep -q "ELF" && strip "$1" && cp "$1" /final/' _ {} \; && \
    # Create the netconfig file (save it as 'netconfig.txt' in the same folder for convenience)
    echo "udp        tpi_clts      v     inet     udp     -          -" >  /final/netconfig.txt && \
    echo "tcp        tpi_cots_ord  v     inet     tcp     -          -" >> /final/netconfig.txt && \
    echo "udp6       tpi_clts      v     inet6    udp     -          -" >> /final/netconfig.txt && \
    echo "tcp6       tpi_cots_ord  v     inet6    tcp     -          -" >> /final/netconfig.txt && \
    echo "rdma       tpi_clts      v     inet     rdma    -          -" >> /final/netconfig.txt && \
    echo "rdma6      tpi_clts      v     inet6    rdma    -          -" >> /final/netconfig.txt

FROM scratch
# The --output command will copy everything from / directly to your host's ./bin folder
